...
| **Risco específico** | Confusão entre faixas de parcelas por número de solicitação — atendente pode aplicar tabela errada ⚠ FP v3 §7 item 15 | Mais grave que na 1ª solicitação: cidadão tem histórico, gera expectativa de número diferente |
...
|---|---|---|---|---|---|
| ⚠1 | Canal errado para iniciar — cidadão que liga para solicitar/renovar é redirecionado sem resolução no canal Caixa | Redirecionamento | A+C | Frontstage | Alta |
| ⚠2 | Queda de ligação antes do atendente humano, sem callback automático relatado | Escalonamento | B | Frontstage | Alta |
| ★3 | **Bloqueio de Senha Cidadão por tentativas DTMF incorretas — caminho de desbloqueio indeterminado (remoto vs. presencial). Risco de severidade mais subestimado do blueprint** | Autenticação | B | Frontstage + Backstage | **Crítica** |
| ⚠4 | Dependência de etapa presencial obrigatória (lotérica) para cadastro e regularização de Senha Cidadão | Autenticação | B | Processos de Suporte | Alta |
| ⚠5 | Divergência de status entre sistemas Caixa e MTE — múltiplas visões dos mesmos dados, *possível defasagem de sincronização não confirmada* | Consulta | B | Backstage | Alta |
